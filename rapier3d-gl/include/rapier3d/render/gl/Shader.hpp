...
class ShaderProgram
{
    public:
        ...
        ShaderProgram(std::string_view vertex_shader_source, std::string_view fragment_shader_source);
        ...
        void UniformTexture(const char * name, Texture & texture) {
            glProgramUniformHandleui64ARB(program, uniform_map.at(name), texture.GetHandle());
        }
